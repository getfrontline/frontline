...
if [[ "$HEDERA_NETWORK" == "testnet" ]]; then
  RPC_URL="${RPC_URL:-https://testnet.hashio.io/api}"
  MIRROR_URL="${MIRROR_URL:-https://testnet.mirrornode.hedera.com}"
else
  RPC_URL="${RPC_URL:-https://mainnet.hashio.io/api}"
  MIRROR_URL="${MIRROR_URL:-https://mainnet.mirrornode.hedera.com}"
fi
...
echo "Deploying Frontline contracts to Hedera ${HEDERA_NETWORK}..." >&2
